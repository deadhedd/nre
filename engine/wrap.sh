...
# Optional wrapper-managed environment file.
#
# Purpose:
# - centralize machine-specific deployment overrides outside cron
# - ensure wrapped jobs get the same environment whether run by cron or manually
#
# Behavior:
# - sourced once, early in wrapper init
# - only if readable
# - path may be overridden by JOB_WRAP_ENV_FILE
# - file must be plain POSIX sh variable assignments / exports
#
# Default location is intentionally outside the repo so local deployment config
# does not live in version-controlled runtime code.
JOB_WRAP_ENV_FILE=${JOB_WRAP_ENV_FILE:-/home/obsidian/nre-private/env.sh}

_wrap_source_env_file() {
  [ -n "${JOB_WRAP_ENV_FILE:-}" ] || return 0
  [ -r "$JOB_WRAP_ENV_FILE" ] || return 0
...
# Load centralized deployment environment after invocation/recursion checks,
# before any wrapper path or runtime defaults are derived.
#
# On failure, treat this as wrapper init failure: the deployment environment is
# part of wrapper-owned runtime configuration.
if ! _wrap_source_env_file; then
  printf '%s\n' "ERROR: WRAP: cannot source env file: ${JOB_WRAP_ENV_FILE:-<unset>}" >&2
  exit "$WRAP_E_INIT"
fi
...
if [ -n "${JOB_WRAP_ENV_FILE:-}" ] && [ -r "$JOB_WRAP_ENV_FILE" ]; then
  _wrap_debug "env-file: loaded $JOB_WRAP_ENV_FILE"
else
  _wrap_debug "env-file: not loaded (missing/unreadable): ${JOB_WRAP_ENV_FILE:-<unset>}"
fi
...
# Optional wrapper-driven status report generation.
# Off by default; enable with WRAP_STATUS_REPORT=1 in the wrapper env file.
WRAP_STATUS_REPORT=${WRAP_STATUS_REPORT:-0}
...
###############################################################################
# Optional report orchestration
###############################################################################

if [ "$WRAP_STATUS_REPORT" -ne 1 ]; then
  _wrap_debug "status-report: disabled"
elif [ "${JOB_NAME:-}" = "script-status-report" ]; then
  _wrap_debug "status-report: skipped for self-report job"
else
  _wrap_info "status-report: generating"
  if "$REPO_ROOT/jobs/helpers/script-status-report-helper.sh"; then
    _wrap_debug "status-report: done"
  else
    _wrap_warn "status-report: helper failed (rc=$?)"
  fi
fi
