...
Options:
  ...
  --vault-log-dir <path>  Destination root for markdown mirrors inside the vault
                          Default: <VAULT_ROOT>/<SERVER_LOGS_DIR>/Latest Logs
...
if [ -z "$vault_log_dir" ]; then
  ...
  # Directory inside the vault for server-log-related notes and mirrors.
  SERVER_LOGS_DIR=${SERVER_LOGS_DIR:-"00 - System/Server Logs"}
  vault_log_dir="${VAULT_ROOT%/}/$SERVER_LOGS_DIR/Latest Logs"
fi
