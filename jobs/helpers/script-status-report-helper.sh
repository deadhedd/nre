#!/bin/sh
# jobs/helpers/script-status-report-helper.sh
# Helper executable for script status report generation.
#
# This helper is wrapper-context only:
# - it must run with wrapper-provided environment (REPO_ROOT, VAULT_ROOT, LOG_ROOT)
# - it must not self-wrap
# - it may be invoked either:
#   1) by jobs/script-status-report.sh (standalone leaf facade), or
#   2) by engine/wrap.sh as post-run helper logic
#
# Author: deadhedd
# License: MIT
# shellcheck shell=sh
...
###############################################################################
# Logging (helper responsibility: emit correctly-formatted messages to stderr)
###############################################################################

# Self-row counters are only used when this helper is running as the standalone
# script-status-report leaf facade.
SELF_WARN=0
SELF_ERR=0
...
if [ -z "${REPO_ROOT:-}" ]; then
  log_error "REPO_ROOT not set (wrapper context required)"
  exit 127
fi
case "$REPO_ROOT" in
  /*) : ;;
...
if [ -z "${VAULT_ROOT:-}" ]; then
  log_error "VAULT_ROOT not set (wrapper context required)"
  exit 127
fi

# Standalone facade mode:
# - emit cadence token for freshness evaluation
# - synthesize the script-status-report self row from the current run
SCRIPT_STATUS_REPORT_STANDALONE=${SCRIPT_STATUS_REPORT_STANDALONE:-0}
SCRIPT_STATUS_REPORT_SELF_ROW=0
if [ "$SCRIPT_STATUS_REPORT_STANDALONE" = "1" ]; then
  SCRIPT_STATUS_REPORT_SELF_ROW=1
...
Options:
  --output <path>   Output file path (absolute).
                    Default: <VAULT_ROOT>/00 - System/Server Logs/Script Status Report.md
...
###############################################################################
# Data/report note locations inside the vault
###############################################################################
SERVER_LOGS_DIR=${SERVER_LOGS_DIR:-"00 - System/Server Logs"}
default_report="$artifact_root/$SERVER_LOGS_DIR/Script Status Report.md"
...
count_matches_ci_ere() {
  ...
  grep -i -E -c "$ere" "$file" 2>/dev/null
}
...
find_previous_completed_log() {
  ...
  _cand=
  for f in "$_search_dir/$_job"*.log; do
    [ -e "$f" ] || continue
    case "$_latest_abs" in
      "$f") continue ;;
    esac
...
generate_report() {
  ...
  cleanup_list() {
    ...
    if [ -n "$latest_issue_tmp" ]; then
      rm -f "$latest_issue_tmp" 2>/dev/null || true
    fi
  }
  ...
  find "$LOG_ROOT" \
    -name '*-latest.log' \
    ! -name 'script-status-report-latest.log' \
    2>/dev/null | sort >"$list_file" || true

  # Optional self row identity (standalone leaf facade only)
  ...
  issue_count=0
  all_scheduled_fresh=1
  latest_issue_tmp=$(mktemp "${TMPDIR:-/tmp}/script-status-issues.XXXXXX") || return 1
  ...
  if [ "$SCRIPT_STATUS_REPORT_SELF_ROW" = "1" ]; then
    # shellcheck disable=SC2016
  ...
  if [ "$SCRIPT_STATUS_REPORT_SELF_ROW" = "1" ]; then
    ...
    if [ "$self_fresh" != "fresh" ] && [ "$self_fresh" != "n/a" ]; then
      all_scheduled_fresh=0
    fi

    if [ "$self_status" != "OK" ] || { [ "$self_fresh" != "fresh" ] && [ "$self_fresh" != "n/a" ]; }; then
      issue_count=$((issue_count + 1))
      printf -- '- ⚠️ %s — status=%s, warns=%s, errs=%s, fresh=%s %s\n' \
        "$self_job" "$self_status" "$self_warn" "$self_err" "$self_fresh" "$self_log_link" \
        >>"$latest_issue_tmp"
    fi
  ...
  while IFS= read -r link; do
    ...
    # Defensive guard: self should already be excluded by find above.
    ...
    if [ -z "$exit_code" ]; then
      ...
      fail_jobs=$((fail_jobs + 1))
      if [ "$fresh_state" != "fresh" ] && [ "$fresh_state" != "n/a" ]; then
        all_scheduled_fresh=0
      fi
      # status_changed unused (intentionally removed)
    fi
    ...
    if [ "$status" != "OK" ] || { [ "$fresh_display" != "fresh" ] && [ "$fresh_display" != "n/a" ]; }; then
      issue_count=$((issue_count + 1))
      printf -- '- ⚠️ %s — status=%s, exit=%s, warns=%s, errs=%s, cadence=%s, fresh=%s %s\n' \
        "$job" "$status" "$exit_display" "$warn_count" "$err_count" "$cadence" "$fresh_display" "$log_link" \
        >>"$latest_issue_tmp"
    fi
  done <"$list_file"
  ...
  if [ "$total_jobs" -eq 0 ]; then
    ...
    printf -- '- **Jobs:** %d total\n' "$total_jobs"
    printf -- '- ✅ %d OK | ⚠️ %d WARN | ❌ %d FAIL | ❓ %d UNKNOWN\n' \
      "$ok_jobs" "$warn_jobs" "$fail_jobs" "$unknown_jobs"
    if [ "$all_scheduled_fresh" -eq 1 ]; then
      printf -- '- 🔄 All scheduled jobs are fresh\n'
    else
      printf -- '- 🔄 Some scheduled jobs need attention\n'
    fi
    ...
    if [ "$issue_count" -gt 0 ]; then
      cat "$latest_issue_tmp"
    else
      printf -- '- ✅ No current issues.\n'
    fi
  fi
