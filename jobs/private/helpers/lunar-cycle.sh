...
moon_guidance() {
  case "$1" in
    "New Moon")         echo "Reset: budget, set 1–3 goals, choose a monthly focus." ;;
    "Waxing Crescent")  echo "Start small: take first reps, schedule the next two actions." ;;
    "First Quarter")    echo "Push through friction: fix blockers, make the hard call." ;;
    "Waxing Gibbous")   echo "Refine: tighten your plan, prep reviews, polish in-progress work." ;;
    "Full Moon")        echo "Mid-month check-in: review, rebalance budget, release dead weight." ;;
    "Waning Gibbous")   echo "Integrate: capture lessons, simplify systems, document what worked." ;;
    "Last Quarter")     echo "Close out: cancel/decline low-ROI tasks, wrap lingering items." ;;
    "Waning Crescent")  echo "Downshift: light maintenance only, prep quietly for next reset." ;;
    *)                  echo "" ;;
  esac
}
...
log_debug "icon=[$(moon_icon "$phase")]"
...
case "$illum_cell" in ""|"(illumination n/a)") illum_cell="n/a" ;; esac
