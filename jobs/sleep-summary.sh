...
# - Input:  "$SLEEP_DATA_DIR/YYYY-MM-DD.txt" (default: 00 - System/Data/Sleep Data)
# - Output: "$SLEEP_DATA_DIR/YYYY-MM-DD Sleep Summary.md" (default: 00 - System/Data/Sleep Data)
#
...
timestamp_to_epoch() {
  ...
  formats='%Y-%m-%dT%H:%M:%S%z|%Y-%m-%dT%H:%M:%S|%Y-%m-%dT%H:%M|%Y-%m-%d %H:%M:%S %z|%Y-%m-%d %H:%M:%S|%Y-%m-%d %H:%M|%b %d, %Y at %I:%M:%S %p|%b %d, %Y at %I:%M %p|%B %d, %Y at %I:%M:%S %p|%B %d, %Y at %I:%M %p|%b %d, %Y at %H:%M:%S|%b %d, %Y at %H:%M|%B %d, %Y at %H:%M:%S|%B %d, %Y at %H:%M'
  old_ifs=$IFS
  IFS='|'
...
log_debug "argv_count=$#"
log_debug "explicit_date=${explicit_date:-<unset>}"
log_debug "output_path=${output_path:-<unset>}"
...
log_debug "env: REPO_ROOT=${REPO_ROOT:-<unset>}"
log_debug "env: VAULT_PATH=${VAULT_PATH:-<unset>}"
log_debug "env: SLEEP_DATA_DIR=${SLEEP_DATA_DIR:-<unset>}"
...
###############################################################################
# Data Notes / sleep data location
###############################################################################
SLEEP_DATA_DIR=${SLEEP_DATA_DIR:-"00 - System/Data/Sleep Data"}

case "$SLEEP_DATA_DIR" in
  /*)
    sleep_folder=$SLEEP_DATA_DIR
    sleep_dir_mode="absolute"
    ;;
  *)
    sleep_folder="$vault_root/$SLEEP_DATA_DIR"
    sleep_dir_mode="vault-relative"
    ;;
esac

log_debug "resolved target_date=$target_date"
log_debug "resolved vault_root=$vault_root"
log_debug "resolved sleep_dir_mode=$sleep_dir_mode"
log_debug "resolved sleep_folder=$sleep_folder"
...
log_info "vault_root: $vault_root"
log_info "sleep_data_dir: $SLEEP_DATA_DIR"
log_info "sleep_folder: $sleep_folder"
log_info "input: $input_path"
log_info "output: $final_output"

if [ ! -d "$sleep_folder" ]; then
  log_error "sleep data directory does not exist: $sleep_folder"
  exit 1
fi
...
if [ ! -f "$input_path" ]; then
  ...
  log_error "cwd=$(pwd -P)"
  log_error "sleep folder contents unavailable check skipped"
...
process_date() {
  ...
  [ -f "$file" ] || return 0
  log_debug "process_date: file=$file"
...
log_debug "split target file: stage_block=$stage_block meta_block=$meta_block"
...
log_debug "wake metadata: yesterday_raw=$yesterday_wake_raw"
log_debug "wake metadata: today_raw=$today_wake_raw"
...
log_debug "wake metadata: yesterday_epoch=$yesterday_epoch"
log_debug "wake metadata: today_epoch=$today_epoch"
...
log_debug "computed total_min=$total_min"
...
log_debug "running average inputs: count=$count avg_min=$avg_min"
...
if [ ! -d "$out_dir" ]; then
  log_error "final_output=$final_output"
